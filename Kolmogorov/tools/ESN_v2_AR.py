...
class ESN_Cell_trainableparams(layers.Layer):
    ...
    def call(self, inputs, states):
        ...
        candidate_state = tf.map_fn(
            lambda x : tf.sparse.sparse_dense_matmul(self.Wres_sp, tf.reshape(x, (x.shape[0], 1))),
            state,
            fn_output_signature=tf.float32,
        )
        candidate_state = tf.squeeze(candidate_state, axis=-1)
        candidate_state = self.activation(self.rho_res * candidate_state + self.omega_in * self.Win(inputs))
...
class ESN(Model):
    ...
    def __init__(
        ...
        self.rnn_list = [
            layers.RNN(
                ...
                batch_size=batch_size if self.stateful else None,
    ...
    def save_model_weights(self, file_name, H5=True):
        ...
        for i in range(len(self.rnn_list)):
            ...
            Wres_this_cell.create_dataset("kernel_values", data=rho_res*Wres_kernel_sparse.values.numpy())
    ...
    def load_weights_from_file(self, file_name):
        ...
        for i in range(len(self.rnn_list)):
            ...
            Win = self.rnn_list[i].cell.Win
            K.set_value(Win.kernel, Win_kernel / self.rnn_list[i].cell.omega_in.numpy())
            if usebias_Win == True:
                Win_bias = np.array(Win_this_cell["bias"])
                K.set_value(Win.bias, Win_bias / self.rnn_list[i].cell.omega_in.numpy())
            ...
            shape = [self.rnn_list[i].cell.state_num_units]*2
            self.rnn_list[i].cell.Wres_sp = tf.sparse.SparseTensor(
                indices=Wres_kernel_sparse_indices,
                values=Wres_kernel_sparse_values / self.rnn_list[i].cell.rho_res.numpy(),
                dense_shape=shape,
            )
