...
# Same as ae_v2, just uses differet padding in the first layer                 #
# Same as ae_v5, uses a specific layer for the weighted multiplication of the  #
...
class single_weights(layers.Layer):
    ...
    def build(self, input_shape):
        self.individual_weights = self.add_weight(
            ...
            shape=[input_shape[-1]],
            initializer=tf.keras.initializers.Constant(value=1.0),#tf.keras.initializers.RandomNormal(mean=1.0, stddev=0.33),
...
class scalar_multiplication(layers.Layer):
    def __init__(self, w_regularizer=None, trainable_scalar=True, init_value=1.0, **kwargs):
        super().__init__()
        self._weights_regularizer = w_regularizer
        self._trainable_scalar = trainable_scalar
        self._init_value = init_value

    def build(self, input_shape):
        self.scalar_multiplier = self.add_weight(
            name='scalar_multiplier',
            shape=(1,),
            initializer=tf.keras.initializers.Constant(value=self._init_value),
            regularizer=self._weights_regularizer,
            trainable=self._trainable_scalar
        )
...
class Autoencoder(Model):
    ...
    def __init__(
        ...
        global_act_iter = 0
        global_padding_iter = 0
        global_conv2d_iter = 0
        global_bn_iter = 0
        global_attn_iter = 0
        global_scalarmult_iter = 0
        global_convtrans_iter = 0
        global_crop_iter = 0
        global_dense_iter = 0
        ...
        for ks_i in range(num_kernel_sizes):
            ...
            for i in range(len_enc_filters):            
                ...
                if self.use_periodic_padding == True:
                    pp_name = 'periodic_padding' + self.create_suffix_from_iter(global_padding_iter)
                    global_padding_iter += 1
                    if i == 0:
                        temp_elp = int(0.5*(self.kernel_size[ks_i]-3))
                        temp_erp = self.kernel_size[ks_i] - temp_elp - 3
                        encoder_layers_list[ks_i].append(periodic_padding(temp_elp, temp_erp, name=pp_name))
                    else:
                        encoder_layers_list[ks_i].append(periodic_padding(elp_lst[ks_i], erp_lst[ks_i], name=pp_name))
                enc_filters_i = self.enc_filters[i]
                enc_stride_i = self.enc_strides[i]
                
                conv_name = 'conv2d' + self.create_suffix_from_iter(global_conv2d_iter)
                global_conv2d_iter += 1
                ...
                if self.use_batch_norm == True:
                    bn_name = 'batch_normalization' + self.create_suffix_from_iter(global_bn_iter)
                    global_bn_iter += 1
                    encoder_layers_list[ks_i].append(layers.BatchNormalization(axis=-3, name=bn_name))
                
                # adding the activation
                act_name = 'activation' + self.create_suffix_from_iter(global_act_iter)
                global_act_iter += 1
                if i == len_enc_filters-1 and self.ls_dim == 0:
                    encoder_layers_list[ks_i].append(layers.Activation(self.enc_final_layer_act_func, name=act_name))
                else:
                    encoder_layers_list[ks_i].append(layers.Activation(enc_layer_activation, name=act_name))
                
                if self.use_attention_module == True:
                    if attn_iter < len(self.enc_attn_placement):
                        if i == self.enc_attn_placement[attn_iter]:
                            # adding the attention module
                            attn_name = 'attention_module' + self.create_suffix_from_iter(global_attn_iter)
                            global_attn_iter += 1
                            encoder_layers_list[ks_i].append(attention_module(att_dim=enc_filters_i, name=attn_name))
            ...
            if self.ls_dim > 0:
                dlayer_name = 'dense' + self.create_suffix_from_iter(global_dense_iter)
                global_dense_iter += 1
                encoder_layers_list[ks_i].append(
                    layers.Dense(
                        units=self.ls_dim,
                ...
                enc_elem_spatial_dims[ks_i].append((1, self.ls_dim))
        ...
        for ks_i in range(num_kernel_sizes):
            scalarmult_name = 'scalar_multiplication' + self.create_suffix_from_iter(global_scalarmult_iter)
            global_scalarmult_iter += 1
            self.encoded_vec_weights.append(scalar_multiplication(trainable_scalar=_trainscalar, init_value=1.0/num_kernel_sizes, name=scalarmult_name))
        ...
        for ks_i in range(num_kernel_sizes):
            ...
            if self.ls_dim > 0:
                x_ks_i = tf.reshape(x_ks_i, [-1, x_ks_i.shape[1]*x_ks_i.shape[2]*x_ks_i.shape[3]])
            x_ks.append(x_ks_i)
        
        encoded_vec = self.encoded_vec_weights[0]( encoder_layers_list[0][-1](x_ks[0]) )
        for ks_i in range(1, num_kernel_sizes):
            encoded_vec += self.encoded_vec_weights[ks_i]( encoder_layers_list[ks_i][-1](x_ks[ks_i]) )


        self.encoder_layers_list = encoder_layers_list
        if self.use_attention_module == True:
            self.encoder_attention_modules_list = encoder_attention_modules_list
        self.encoder_net = Model(inputs=input_vec, outputs=encoded_vec, name='model')
        ...
        print('enc_elem_spatial_dims : ', enc_elem_spatial_dims)
        print('dec_elem_spatial_dims : ', dec_elem_spatial_dims)
        for ks_i in range(num_kernel_sizes):
            attn_iter = 0
            if self.ls_dim > 0:
                dlayer_name = 'dense' + self.create_suffix_from_iter(global_dense_iter)
                global_dense_iter += 1
                dlayer_units = enc_elem_spatial_dims[ks_i][-2][0]*enc_elem_spatial_dims[ks_i][-2][1]*self.enc_filters[-1]
                ...
                dec_elem_spatial_dims[ks_i].append((enc_elem_spatial_dims[ks_i][-2][0], enc_elem_spatial_dims[ks_i][-2][1]))
            for i in range(len_dec_filters):
                # print('{} -- kernelsize : {} -- {} / {}'.format(ks_i, self.kernel_size[ks_i], i, len_dec_filters-1))
                # periodic padding
                pp_name = 'periodic_padding' + self.create_suffix_from_iter(global_padding_iter)
                global_padding_iter += 1
                decoder_layers_list[ks_i].append(periodic_padding(elp_lst[ks_i], erp_lst[ks_i], name=pp_name,))
                # transposed CNN
                dec_filters_i = self.dec_filters[i]
                dec_stride_i = self.dec_strides[i]
                
                convtrans_name = 'conv2d_transpose' + self.create_suffix_from_iter(global_convtrans_iter)
                global_convtrans_iter += 1
                decoder_layers_list[ks_i].append(
                ...
                if i == len(self.dec_filters)-1:
                    ...
                    cb = total_crop - ct
                    cr = cb
                crop_name = 'cropping2d' + self.create_suffix_from_iter(global_crop_iter)
                global_crop_iter += 1
                decoder_layers_list[ks_i].append(
                    layers.Cropping2D(
                        cropping=(
                ...
                if self.use_batch_norm == True:
                    bn_name = 'batch_normalization' + self.create_suffix_from_iter(global_bn_iter)
                    global_bn_iter += 1
                    decoder_layers_list[ks_i].append(layers.BatchNormalization(axis=-3, name=bn_name))
                # adding the activation
                act_name = 'activation' + self.create_suffix_from_iter(global_act_iter)
                global_act_iter += 1
                if i == len(self.dec_filters)-1:
                    decoder_layers_list[ks_i].append(layers.Activation(self.dec_final_layer_act_func, name=act_name))
                else:
                    decoder_layers_list[ks_i].append(layers.Activation(dec_layer_activation, name=act_name))
                if self.use_attention_module == True:
                    if attn_iter < len(self.dec_attn_placement):
                        if i == self.dec_attn_placement[attn_iter]:
                            ...
                            attn_name = 'attention_module' + self.create_suffix_from_iter(global_attn_iter)
                            global_attn_iter += 1
                            decoder_layers_list[ks_i].append(attention_module(att_dim=dec_filters_i, name=attn_name))
        ...
        for ks_i in range(num_kernel_sizes):
            scalarmult_name = 'scalar_multiplication' + self.create_suffix_from_iter(global_scalarmult_iter)
            global_scalarmult_iter += 1
            self.decoded_vec_weights.append(scalar_multiplication(trainable_scalar=_trainscalar, init_value=1.0/num_kernel_sizes, name=scalarmult_name))
        
        y_ks = []
        for ks_i in range(num_kernel_sizes):
            y_ks_i = decoder_layers_list[ks_i][0](encoded_input_vec)
            if self.ls_dim > 0:
                y_ks_i = tf.reshape(y_ks_i, (-1, self.enc_filters[-1], enc_elem_spatial_dims[ks_i][-2][0], enc_elem_spatial_dims[ks_i][-2][1]))
            for i in range(1, len(decoder_layers_list[ks_i])-1):
                y_ks_i = decoder_layers_list[ks_i][i](y_ks_i)
            y_ks.append(y_ks_i)
        
        decoded_vec = self.decoded_vec_weights[0]( decoder_layers_list[0][-1](y_ks[0]) )
        for ks_i in range(1, num_kernel_sizes):
            decoded_vec += self.decoded_vec_weights[ks_i]( decoder_layers_list[ks_i][-1](y_ks[ks_i]) )

        
        if self.use_weights_post_dense == True:
            decoder_layers_list[0].append(
                single_weights(w_regularizer=reg(self.lambda_reg), name='single_weights')
            )
        ...
        self.decoder_net = Model(inputs=encoded_input_vec, outputs=decoded_vec, name='model_1')

        ### all combined, the autoencoder
        input_vec_z = Input(input_vec.shape[-3:])
        z = self.encoder_net(input_vec_z)
        z = self.decoder_net(z)
        self.ae_net = Model(inputs=input_vec_z, outputs=z, name='model_2')
    ...
    def create_suffix_from_iter(self, _iter):
        suffix = ''
        if _iter > 0:
            suffix = '_' + str(int(_iter))
        return suffix
    ...
    def train_step(self, data):
        ...
        for ks_i in range(self.num_kernel_sizes):
            l = self.encoded_vec_weights[ks_i]
            kwargs['{}__encoded_vec_wt'.format(ks_i)] = l.scalar_multiplier
        for ks_i in range(self.num_kernel_sizes):
            l = self.decoded_vec_weights[ks_i]
            kwargs['{}__decoded_vec_wt'.format(ks_i)] = l.scalar_multiplier
