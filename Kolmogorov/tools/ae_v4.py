################################################################################
# Deep denoising 2D-CNN autoencoder.                                           #
# Uses a single weights layer at the end.                                      #
# No periodic padding, only zero-padded                                        #
...
class Autoencoder(Model):
    ...
    def __init__(
        ...
        enc_elem_spatial_dims = [prev_shape]
        ...
        for i in range(len_enc_filters):            
            ...
            if self.use_periodic_padding == True:
                if i == 0:
                    temp_elp = int(0.5*(self.kernel_size-3))
                    temp_erp = self.kernel_size - temp_elp - 3
                    encoder_layers_list.append(periodic_padding(temp_elp, temp_erp))
                else:
                    encoder_layers_list.append(periodic_padding(elp, erp))
            enc_filters_i = self.enc_filters[i]
            enc_stride_i = self.enc_strides[i]
            encoder_layers_list.append(layers.Conv2D(
            ...
            if self.use_batch_norm == True:
                encoder_layers_list.append(layers.BatchNormalization(axis=-3))
            # adding the activation
            encoder_layers_list.append(layers.Activation(enc_layer_activation))
            
            if self.use_attention_module == True:
                if attn_iter < len(self.enc_attn_placement):
                    if i == self.enc_attn_placement[attn_iter]:
                        # adding the attention module
                        encoder_layers_list.append(attention_module(att_dim=enc_filters_i))
                        encoder_attention_modules_list.append(encoder_layers_list[-1])
                        attn_iter += 1
            
            in_spatial_dims = enc_elem_spatial_dims[-1]
            in_spatial_dims_y = in_spatial_dims[0] + self.kernel_size-1 # because padding is either periodic or `same`
            in_spatial_dims_x = in_spatial_dims[1] + self.kernel_size-1 # because padding is either periodic or `same`
            enc_elem_spatial_dims.append(
        ...
        for i in range(len(encoder_layers_list)-1):
        ...
        dec_elem_spatial_dims = [encoded_vec.shape[-2:]]
        attn_iter = 0
        ...
        for i in range(len_dec_filters):
            ...
            # center-crop
            in_spatial_dims = dec_elem_spatial_dims[-1]
            in_spatial_dims_y = in_spatial_dims[0] + self.kernel_size-1 # because padding is either periodic or `same`
            in_spatial_dims_x = in_spatial_dims[1] + self.kernel_size-1 # because padding is either periodic or `same`
            ...
            if i == len(self.dec_filters)-1:
                ...
                if self.kernel_size == 3:
                    total_crop = int((dec_stride_i+1)*(self.kernel_size-1) - (dec_stride_i-1))
                else:
                    total_crop = int((dec_stride_i+1)*(self.kernel_size-1))
            ...
            dec_elem_spatial_dims.append(
                (out_spatial_dims_y, out_spatial_dims_x)
            )
            # batch normalization
            if self.use_batch_norm == True:
                decoder_layers_list.append(layers.BatchNormalization(axis=-3))
            # adding the activation
            decoder_layers_list.append(layers.Activation(self.dec_layer_act_func))
            if self.use_attention_module == True:
                if attn_iter < len(self.dec_attn_placement):
                    if i == self.dec_attn_placement[attn_iter]:
                        # adding the attention module
                        decoder_layers_list.append(attention_module(att_dim=dec_filters_i))
                        decoder_attention_modules_list.append(decoder_layers_list[-1])
                        attn_iter += 1
        ...
        decoded_vec = decoder_layers_list[-1](y)
        ...
        if self.use_weights_post_dense == True:
            ...
            decoded_vec = decoder_layers_list[-1](decoded_vec)
    ...
    def compute_post_conv_shape(self, input_shape, stride):
        out_x = int( (input_shape[1]-self.kernel_size) // stride) + 1
        out_y = int( (input_shape[0]-self.kernel_size) // stride) + 1
        return (out_y, out_x)

    def compute_post_tconv_shape(self, input_shape, stride):
        out_x = (input_shape[1]-1)*stride + self.kernel_size
        out_y = (input_shape[0]-1)*stride + self.kernel_size
        return (out_y, out_x)
    ...
    def train_step(self, data):
        ...
        kwargs = {}
        if self.use_attention_module == True:
            for i in range(len(self.encoder_attention_modules_list)):
                l = self.encoder_attention_modules_list[i]
                kwargs['encoder_attention_module_{}_lambda'.format(i)] = l.lambda_att
            for i in range(len(self.decoder_attention_modules_list)):
                l = self.decoder_attention_modules_list[i]
                kwargs['decoder_attention_module_{}_lambda'.format(i)] = l.lambda_att
