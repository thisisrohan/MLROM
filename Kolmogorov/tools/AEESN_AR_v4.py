...
class AR_AERNN_ESN(Model):
    ...
    def __init__(
        ...
        input_enc = Input(shape=(np.prod(self.time_stddev_ogdata.shape), ))
        x_enc = tf.reshape(input_enc, (-1,)+tuple(self.time_stddev_ogdata.shape))
        x_enc = self.ae_net.encoder_net(x_enc)        
        self.td_encoder_net = Model(inputs=input_enc, outputs=x_enc)
        self.ls_shape = tuple(x_enc.shape[1:])
        
        input_dec = Input(shape=(np.prod(x_enc.shape[-3:]),))
        x_dec = tf.reshape(input_dec, (-1,) + tuple(x_enc.shape[-3:]))
        x_dec = self.ae_net.decoder_net(x_dec)
        self.td_decoder_net = Model(inputs=input_dec, outputs=x_dec)
        
        temp = tf.ones(shape=(self.rnn_net.batch_size,)+(np.prod(self.time_stddev_ogdata.shape), ))
        temp = self.td_encoder_net(temp)
        del(temp)
        temp = tf.ones(shape=(self.rnn_net.batch_size,)+(np.prod(x_enc.shape[-3:]), ))
        temp = self.td_decoder_net(temp)
        del(temp)
    ...
    # @tf.function
    def _warmup(self, inputs, training=None, usenoiseflag=None, **kwargs):
        ...
        for i in range(inputs.shape[1]):
            x.append(self.ae_net.encoder_net(inputs[:, i], training=training))
        x = tf.stack(x)
        ...
        output = tf.reshape(output, (-1, output.shape[1]) + tuple(self.ls_shape))
        y = []
        for i in range(output.shape[1]):
            y.append(self.ae_net.decoder_net(output[:, i], training=training))
        y = tf.stack(y)
        y = tf.transpose(y, [1, 0, 2, 3, 4])
        

        if len(rnnwarmup_return_tuple) == 2:
            return_tuple = (y, states_list)
        else:
            return_tuple = (y, states_list, intermediate_outputs_lst, scalar_multiplier_list)
    ...
    def save_everything(self, file_name, H5=True):
        ...
        self.rnn_net.save_class_dict(file_name+'_ESN_class_dict.txt')

        ### saving weights
        self.ae_net.save_model_weights(file_name+'_ae_weights', H5=H5)
        self.rnn_net.save_model_weights(file_name+'_ESN_weights', H5=H5)
