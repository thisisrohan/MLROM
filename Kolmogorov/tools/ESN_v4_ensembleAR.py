...
class ESN_Cell(layers.Layer):
    def __init__(
            ...
            usebias_Win=False, prng_seed=42, activation='tanh', wts_to_be_loaded=False, **kwargs):

        super(ESN_Cell, self).__init__()

        # wts_to_be_loaded = kwargs.pop('wts_to_be_loaded', False)
        ...
        self.Win_sp = None
        self.Win_bias = None
        self.Wres_sp = self._build_Wres(wts_to_be_loaded)
        
        return

    def _build_Win(self, wts_to_be_loaded=False):
    ...
    def _build_Wres(self, wts_to_be_loaded=False):
        ...
        if wts_to_be_loaded == False:
            spectral_rad = np.abs(sp_la.eigs(Wres, k=1, which='LM', return_eigenvectors=False))[0]
        else:
            spectral_rad = 1.0
            # just making a dummy Wres matrix since wts are to be loaded on later
    ...
    def build(self, input_shape, wts_to_be_loaded=False):
    
        self.input_size = input_shape[-1]

        self.Win_sp, self.Win_bias = self._build_Win(wts_to_be_loaded)
        if self.usebias_Win == True:
...
class ESN_ensemble(Model):
    ...
    def __init__(
        ...
        if lambda_reg != None:
            for i in range(len(self.lambda_reg)):
                self.lambda_reg[i] = lambda_reg
            ...
            rnn_list = [
                layers.RNN(
                    cell=ESN_Cell(
                        ...
                        wts_to_be_loaded=wts_to_be_loaded,
                    ),
    ...
    def build(self, input_shape):
        for i_en in range(len(self.load_files_lst)):
            ...
            for i_layer in range(len(self.rnn_list[i_en])):
                l = self.rnn_list[i_en][i_layer]
                l.build((int(self.batch_size), int(input_shape[-2]), int(ldim)))
                ldim = self.ESN_layers_units[i_en][i_layer]
            self.Wout[i_en].build((int(self.batch_size), int(ldim)))
            if self.use_weights_post_dense[i_en] == True:
                self.postWout[i_en].build((int(self.batch_size), int(ldim)))
        self.built = True
        return
    ...
    def save_model_weights(self, file_name, H5=True):
        ...
        for i_file in range(len(self.load_files_lst)):
            ...
            for i in range(len(self.rnn_list[i_file])):
                ...
                Win_kernel_sparse = self.rnn_list[i_file][i].cell.Win_sp
                ...
                if usebias_Win == True:
                    Win_bias = self.rnn_list[i_file][i].cell.Win_bias
                    Win_this_cell.create_dataset("bias", data=Win_bias.numpy())
                    
                ### saving Wres in a sparse manner
                Wres_kernel_sparse = self.rnn_list[i_file][i].cell.Wres_sp
    ...
    def load_weights_from_file(self, file_name_lst):
        ...
        for i_file in range(len(file_name_lst)):
            ...
            for i in range(len(self.rnn_list[i_file])):
                ...
                self.rnn_list[i_file][i].cell.Win_sp = tf.sparse.SparseTensor(
                    indices=Win_kernel_sparse_indices,
                    values=Win_kernel_sparse_values,
                    dense_shape=[input_dim, self.ESN_layers_units[i_file][i]])
                if usebias_Win == True:
                    self.rnn_list[i_file][i].cell.Win_bias =  tf.constant(np.array(Win_this_cell["bias"]))
                ...
                self.rnn_list[i_file][i].cell.Wres_sp = tf.sparse.SparseTensor(
