...
class NMSE(tf.keras.metrics.MeanSquaredError):
    def __init__(self, divisor_arr, name='NMSE', **kwargs):
        ...
        self.divisor_arr = tf.constant(np.array(divisor_arr, dtype='float32'))
...
class AERNN_loss(tf.keras.losses.Loss):
    def __init__(self, divisor_arr, loss_weights=None):
        ...
        self.divisor_arr = tf.constant(np.array(divisor_arr, dtype=np.float32))
        self.loss_weights = loss_weights
        if loss_weights is not None:
            self.loss_weights = tf.constant(np.array(loss_weights**0.5, dtype=np.float32))
...
def trainAERNN(
    ...
    if isinstance(loss_weights, list) == False:
        ...
        loss_weights = loss_weights.reshape((loss_weights.shape[0], ) + tuple(all_data_og_shape[1:]))
    # print('loss_weights : ', loss_weights)
    
    print(data_rnn_input.shape, data_rnn_output.shape)
    data_rnn_input = data_rnn_input.reshape((data_rnn_input.shape[0], data_rnn_input.shape[1]) + tuple(all_data_og_shape[1:]))
    data_rnn_output = data_rnn_output.reshape((data_rnn_output.shape[0], data_rnn_output.shape[1]) + tuple(all_data_og_shape[1:]))
    ...
    try:
        ndims2 = len(rnn_net.data_dim)
    except:
        rnn_dd = (rnn_net.data_dim,)

    if behaviour == 'initialiseAndTrainFromScratch':
        rnn_net.save_class_dict(save_path+'/final_net_class_dict.txt')
        rnn_net.build(input_shape=(batch_size, training_data_rnn_input.shape[1],) + tuple(rnn_dd))        
    ...
    ### AE-RNN
    if type(normalization_constant_arr_rnndata) != type(None):
        ndims1 = len(normalization_constant_arr_rnndata.shape) - 1
        ndims2 = len(rnn_dd)
        if ndims1 != ndims2:
            normalization_constant_arr_rnndata = normalization_constant_arr_rnndata.reshape((2,)+tuple(rnn_dd))
    ...
    #             validation_split=val_split/train_split,
            ...
            if type(rnn_net.train_rho_res) != type(None):
                temp = []
                for jj in range(len(AR_AERNN_net.rnn_net.rnn_list)):
                    try:
                        tr_jj = rnn_net.train_rho_res[jj]
                    except:
                        try:
                            tr_jj = rnn_net.train_rho_res[0]
                        except:
                            tr_jj = rnn_net.train_rho_res
                    if tr_jj == True:
                        temp.append(history.history['rho_res_{}'.format(jj)])
                if len(temp) > 0:
                    rho_res_hist.append(temp)
            # if 'alpha_0' in history.history.keys():
            if type(rnn_net.train_alpha) != type(None):
                temp = []
                for jj in range(len(AR_AERNN_net.rnn_net.rnn_list)):
                    try:
                        tr_jj = rnn_net.train_alpha[jj]
                    except:
                        try:
                            tr_jj = rnn_net.train_alpha[0]
                        except:
                            tr_jj = rnn_net.train_alpha
                    if tr_jj == True:
                        temp.append(history.history['alpha_{}'.format(jj)])
                if len(temp) > 0:
                    alpha_hist.append(temp)
            # if 'omega_in_0' in history.history.keys():
            if type(rnn_net.train_omega_in) != type(None): 
                temp = []
                for jj in range(len(AR_AERNN_net.rnn_net.rnn_list)):
                    try:
                        tr_jj = rnn_net.train_omega_in[jj]
                    except:
                        try:
                            tr_jj = rnn_net.train_omega_in[0]
                        except:
                            tr_jj = rnn_net.train_omega_in
                    if tr_jj == True:
    ...
    plt.savefig(dir_name_plot+'/loss_history-{}_outsteps.png'.format(num_outsteps), dpi=300, bbox_inches='tight')
    plt.show()
    plt.clf()
    ...
    plt.savefig(dir_name_plot+'/MSE_history-{}_outsteps.png'.format(num_outsteps), dpi=300, bbox_inches='tight')
    plt.show()
    plt.clf()
    ...
    plt.savefig(dir_name_plot+'/NMSE_history-{}_outsteps.png'.format(num_outsteps), dpi=300, bbox_inches='tight')
    plt.show()
    plt.clf()
    ...
    fig, ax = plot_losses(
        training_loss=train_covmat_fro_loss_hist,
        val_loss=None,
        lr_change=lr_change,
        learning_rate_list=learning_rate_list,
        legend_list=['train_covmat_fro_loss_hist'],
        xlabel='Epoch',
        ylabel='covmat_fro_loss',
        plot_type='plot',
    )
    plt.savefig(dir_name_plot+'/train_covmat_fro_loss_hist_history-{}_outsteps.png'.format(num_outsteps), dpi=300, bbox_inches='tight')
    plt.show()
    plt.clf()
