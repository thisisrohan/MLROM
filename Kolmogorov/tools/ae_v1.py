...
class Autoencoder(Model):
    ...
    def __init__(
            ...
            kernel_size=3,
            enc_filters=[8, 16, 32, 3], # number of filters
            dec_filters=[32, 16, 8, 2], # number of filters
            lambda_reg=0.0,
            reg_name='L2',
        ...
        encoder_layers_list = []
        encoder_left_paddings = []
        encoder_right_paddings = []
        ...
        for i in range(len_enc_filters):
            ...
            encoder_left_paddings.append(elp)
            encoder_right_paddings.append(erp)
            
            # adding the conv layer
            if self.use_periodic_padding == True:
                encoder_layers_list.append(periodic_padding(elp, erp))
            enc_filters_i = self.enc_filters[i]
            if i == len_enc_filters-1 and len_enc_filters >= 2:
                enc_filters_i = self.enc_filters[-2]
            ...
            if i == 0 and len_enc_filters >= 2:
                pass
            elif i < len_enc_filters-1:
                if self.use_attention_module == True:
                    # adding the attention module
                    encoder_layers_list.append(attention_module(att_dim=self.enc_filters[i]))
                    encoder_attention_modules_list.append(encoder_layers_list[-1])
                else:
                    pass
            elif i == len_enc_filters-1:
                # adding the final stride-1 CNN to take to final dimensions
                encoder_left_paddings.append(elp)
                encoder_right_paddings.append(erp)
                if self.use_periodic_padding == True:
                    encoder_layers_list.append(periodic_padding(elp, erp))
                encoder_layers_list.append(layers.Conv2D(
                    filters=self.enc_filters[-1],
                    kernel_size=self.kernel_size,
                    # padding='valid',
                    # padding='same',
                    padding=padding,
                ...
                if self.use_batch_norm == True:
                    encoder_layers_list.append(layers.BatchNormalization(axis=-3))
                # adding the activation
                encoder_layers_list.append(layers.Activation(self.enc_final_layer_act_func))
                if self.use_attention_module == True:# and len_enc_filters >= 2:
                    # adding the attention module
                    encoder_layers_list.append(attention_module(att_dim=self.enc_filters[i], v_channel_dim=self.enc_filters[-1]))
                    encoder_attention_modules_list.append(encoder_layers_list[-1])
                else:
                    pass
        ...
        decoder_layers_list = [
            ...
            layers.Cropping2D(
                cropping=(
                    (encoder_left_paddings[-1], encoder_right_paddings[-1]),
                    (encoder_left_paddings[-1], encoder_right_paddings[-1])
                ),
                data_format='channels_first'
            )
        ...
        for i in range(len(self.dec_filters)):
            ...
            if i < len(self.dec_filters)-2:
                if self.use_attention_module == True:
                    # adding the attention module
                    decoder_layers_list.append(attention_module(att_dim=self.dec_filters[i]))
        ...
        input_vec_z = Input(input_vec.shape[1:])
