...
class Autoencoder(Model):
    ...
    def __init__(
        ...
        for ks_i in range(num_kernel_sizes):
            ...
            for i in range(len_dec_filters):
                ...
                act_name = 'activation' + self.create_suffix_from_iter(global_act_iter)
                global_act_iter += 1
                decoder_layers_list[ks_i].append(layers.Activation(self.dec_layer_act_func, name=act_name))
                if self.use_attention_module == True:
    ...
    def create_enc_wt(self, init_value, trainable):
        scalarmult_name = 'scalar_multiplication' + self.create_suffix_from_iter(self.global_scalarmult_iter)
        self.global_scalarmult_iter += 1
        self.encoded_vec_layers.append(scalar_multiplication(trainable_scalar=trainable, init_value=init_value, name=scalarmult_name))
        self.encoded_vec_layers[-1].build([])

    def create_dec_wt(self, init_value, trainable):
        scalarmult_name = 'scalar_multiplication' + self.create_suffix_from_iter(self.global_scalarmult_iter)
        self.global_scalarmult_iter += 1
        self.decoded_vec_layers.append(scalar_multiplication(trainable_scalar=trainable, init_value=init_value, name=scalarmult_name))
        self.decoded_vec_layers[-1].build([])
    ...
    def update_models(self):
        ...
        for ks_i in range(self.num_kernel_sizes):
            x_ks_i = self.encoder_layers_list[ks_i][0](x)
            for i in range(1, len(self.encoder_layers_list[ks_i])-1):
                x_ks_i = self.encoder_layers_list[ks_i][i](x_ks_i)
            x_ks.append(x_ks_i)
        
        encoded_vec = self.encoded_vec_layers[0]( self.encoder_layers_list[0][-1](x_ks[0]) )
        for ks_i in range(1, self.num_kernel_sizes):
            encoded_vec += self.encoded_vec_layers[ks_i]( self.encoder_layers_list[ks_i][-1](x_ks[ks_i]) )
        ...
        y_ks = []
        for ks_i in range(self.num_kernel_sizes):
            y_ks_i = self.decoder_layers_list[ks_i][0](encoded_input_vec)
            for i in range(1, len(self.decoder_layers_list[ks_i])-1):
                y_ks_i = self.decoder_layers_list[ks_i][i](y_ks_i)
            y_ks.append(y_ks_i)
        
        decoded_vec = self.decoded_vec_layers[0]( self.decoder_layers_list[0][-1](y_ks[0]) )
        for ks_i in range(1, self.num_kernel_sizes):
            decoded_vec += self.decoded_vec_layers[ks_i]( self.decoder_layers_list[ks_i][-1](y_ks[ks_i]) )

        
        if self.use_weights_post_dense == True:
            y_shape = decoded_vec.shape
            decoded_vec = tf.reshape(decoded_vec, (-1, y_shape[-3]*y_shape[-2]*y_shape[-1]))
            decoded_vec = self.decoder_layers_list[0][-1](decoded_vec)
            decoded_vec = tf.reshape(decoded_vec, y_shape)
