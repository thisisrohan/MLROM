...
class Autoencoder(Model):
    ...
    def compute_normed_grad(self, x):
    
        with tf.GradientTape() as tape1:
            with tf.GradientTape(watch_accessed_variables=False, persistent=False) as tape2:
                tape2.watch(x)
                tape2.watch(self.trainable_variables)
                encoded = self.encoder_net(x, training=True)
                
            # encoded_norm = tf.math.reduce_mean(encoded**2, axis=-1, keepdims=True)**0.5
            ls_norm = tf.constant(np.array((tf.norm(encoded, axis=-1, keepdims=True))))
            ls_jacobian = tape2.batch_jacobian(encoded, x)
            ...
            ls_jacobian = tf.reduce_mean(tf.norm(
                ls_jacobian,
                ord='fro',
                axis=[-2, -1]
            ))
            ...
            #     end_idx = int(len(x)*(1+i)/num_runs)
            #     ls_jacobian_norm += tf.reduce_mean(tf.norm(
            #         ls_jacobian[begin_idx:end_idx],
            #         ord='fro',
            #         axis=[-2, -1]
            #     ) / ls_norm[begin_idx:end_idx])
            #     ls_jacobian += tf.reduce_mean(tf.norm(
            #         ls_jacobian[begin_idx:end_idx],
            #         ord='fro',
            #         axis=[-2, -1]
            ...
            #     begin_idx = end_idx
            # 
            # ls_jacobian_norm /= num_runs
            # ls_jacobian /= num_runs
        ...
        return np.array(ls_jacobian_norm), np.array(ls_jacobian)
