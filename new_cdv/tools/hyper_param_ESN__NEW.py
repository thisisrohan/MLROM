...
from .ESN_v1_ensembleAR import ESN_ensemble as ESN_ensemble_AR
...
def trainESN_and_return_PH(
    ...
    load_files_list = [save_path+dir_sep+'{}_final_net_class_dict.txt'.format(i) for i in range(num_ensemble_mems)]
    load_wts_list = [save_path+dir_sep+'{}_final_net_ESN_weights.hdf5'.format(i) for i in range(num_ensemble_mems)]
    ensemble_ESN_AR_pred = ESN_ensemble_AR(num_sample_output_AR, load_files_lst=load_files_list)
    ensemble_ESN_AR_pred.build([data_in.shape[0], num_sample_input_AR, data_in.shape[2]])
    ensemble_ESN_AR_pred.load_weights_from_file(load_wts_list)
    build_time = time.time() - build_time
    print('    build_time : {:.0f}h {:.0f}m {:.0f}s'.format(
        float(build_time // 3600),
        float((build_time%3600)//60),
        float((build_time%3600)%60),
    ))
    ...
    for i in range(num_runs):
        ...
        for rnn_list in ensemble_ESN_AR_pred.ESN_layers:
            for layer in rnn_list:
                layer.reset_states()

        prediction_lst = np.array(ensemble_ESN_AR_pred(data_[:, 0:num_sample_input_AR, :]))[0]
