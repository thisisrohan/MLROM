...
class SimpleRNNCell_zoneout(layers.SimpleRNNCell):
    def __init__(self, **kwargs):
        ...
        super(SimpleRNNCell_zoneout, self).__init__(**kwargs)

    def call(self, inputs, states, training=None):
        h_tm1 = (
            states[0] if tf.nest.is_nested(states) else states
        )
        output, candidate_states = super(SimpleRNNCell_zoneout, self).call(
            inputs,
            states,
            training
        )
...
class AR_RNN_SimpleRNN(Model):
    ...
    def __init__(
        ...
        super(AR_RNN_SimpleRNN, self).__init__()
        ...
        if self.num_skip_connections > 0:
            self.RK_RNNCell = cell=SimpleRNNCell_zoneout(
