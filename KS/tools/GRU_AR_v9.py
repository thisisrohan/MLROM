...
# The z gate uses h_tilde values instead of h_tm1                              #
# For GRU_SingleStep_v14                                                       #
#------------------------------------------------------------------------------#
...
class GRUCell_zoneout(layers.GRUCell):
    ...
    def call(self, inputs, states, training=None):
        ...
        if self.implementation == 1:
            ...
            # r_rowsum = tf.math.reduce_sum(r, axis=-1, keepdims=True)
            # r = r / r_rowsum
            ...
            if 0.0 < self.recurrent_dropout < 1.0:
                hh_z = hh * rec_dp_mask[0]
            else:
                hh_z = hh
            recurrent_z = K.dot(
                hh_z, self.recurrent_kernel[:, : self.units]
            )
            if self.use_bias:
            ...
            # z_htm1 = 1 - z
            # z_rowsum = tf.math.reduce_sum(z, axis=-1, keepdims=True)
            # z = z / z_rowsum
            # z_htm1_rowsum = tf.math.reduce_sum(z_htm1, axis=-1, keepdims=True)
            # z_htm1 = z_htm1 / z_htm1_rowsum
        else:
            if 0.0 < self.dropout < 1.0:
            ...
            if self.reset_after:
                ...
                if self.use_bias:
                    matrix_inner = K.bias_add(
                        matrix_inner, recurrent_bias#[self.units:]
                    )
            else:
            ...
            _, recurrent_r, recurrent_h = tf.split(
            ...
            recurrent_z = K.dot(hh, self.recurrent_kernel[:, 0:self.units])
            if self.reset_after and self.use_bias:
                recurrent_z = K.bias_add(
                    recurrent_z, recurrent_bias[0:self.units]
                )
            z = self.recurrent_activation(x_z + recurrent_z)
            # z_htm1 = 1 - z
            # z_rowsum = tf.math.reduce_sum(z, axis=-1, keepdims=True)
        ...
        h = (1 - z) * h_tm1 + z * hh
        candidate_states = [h] if tf.nest.is_nested(states) else h
        output = h
...
class AR_RNN_GRU(Model):
    ...
    def __init__(
        ...
        # self.init_state = []
        # for i in range(self.rnn_list):
        #     self.init_state.append(
        #         self.rnn_list[i].cell.get_initial_state
        #     )
        ...
        # elif
        if self.use_learnable_state == True and self.stateful == False:
            x_in = tf.ones(
    ...
    def train_step(self, data):
        ...
        return self.compute_metrics(x, y, ypred, sample_weight)
