...
class CrossAttentionSimpleRNNCell_zoneout(CrossAttentionSimpleRNNCell):
    def __init__(self, **kwargs):
        self.zoneout_rate = kwargs.pop('zoneout_rate', 0.0)
        super(CrossAttentionSimpleRNNCell_zoneout, self).__init__(**kwargs)

    def call(self, inputs, states, training=None):
        h_tm1 = (
            states[0] if tf.nest.is_nested(states) else states
        )
        output, candidate_states = super(CrossAttentionSimpleRNNCell_zoneout, self).call(
            inputs,
            states,
            training
        )
...
class AR_RNN_CrossAttentionSimpleRNN(Model):
    """
    ...
    def __init__(
        ...
        super(AR_RNN_CrossAttentionSimpleRNN, self).__init__()
        ...
        self.rnn_list = [
            layers.RNN(
                cell=CrossAttentionSimpleRNNCell_zoneout(
        ...
        if self.num_skip_connections > 0:
            self.RK_RNNCell = cell=CrossAttentionSimpleRNNCell_zoneout(
