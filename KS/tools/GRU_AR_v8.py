...
# For GRU_SingleStep_v13                                                       #
#------------------------------------------------------------------------------#
#                        Basic Network Architecture                            #
#------------------------------------------------------------------------------#
#                                                                              #
#                         ______________________________________               #
#                        /                  _\ _________________\           u  #
#                       /                  /  \         u        \    +a13*d1  #
#                      /             u    /    \  +a12*d1         \   +a23*d2  #
#                   __/  d1    +a11*d1 __/  d2  \ +a22*d2 __   d3  \  +a33*d3  #
#          u------>|__|----->[+]----->|__|----->[+]----->|__|----->[+]----->   #
#           \________________/                  /                  /           #
...
#                                                                              #
# The RNN weights are shared amongst the 1st, 2nd,... networks. Need to        #
...
class AR_RNN_GRU(Model):
    ...
    def __init__(
        ...
        self.num_skip_connections = self.num_rnn_layers
    ...
    def _warmup(self, inputs, training=None, usenoiseflag=False, **kwargs):
        ...
        # remaining layers
        for i in range(self.num_skip_connections):
            prediction, states = self.rnn_list[i](
                x,
                training=training,
            )
            ...
            for j in range(len(self.dense_layer_act_func)):
                if self.denselayer_dropout_rate > 0.0:
                    prediction = self.dense_dropout[j](prediction, training=training)
                prediction = layers.TimeDistributed(self.dense[j])(prediction, training=training)
            if self.use_weights_post_dense == True:
                prediction = layers.TimeDistributed(self.dense[-1])(prediction, training=training)
            prediction = self.dt_rnn*prediction
