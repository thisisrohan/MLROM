################################################################################
# RK Methods inspired residual POD-RNN with skip connections, with             #
...
class AR_POD_RNN(Model):
    ...
    def __init__(
        ...
        self.reverseNormalization_postPOD = reverseNormalization_layer(pod_data_normalization_arr)
        self.pod_centering_means = pod_centering_means
    ...
    def _warmup(self, inputs, training=None, usenoiseflag=None):
        ...
        # first layer
        x = inputs - self.pod_centering_means
        x = tf.linalg.matmul(x, self.Wl)
        ...
        output = tf.linalg.matmul(output, self.Wl.transpose()) + self.pod_centering_means
    ...
    def call(self, inputs, training=None, usenoiseflag=False):
        ...
        for tstep in range(1, self.rnn_net.out_steps):
            ...
            x = tf.linalg.matmul(x, self.Wl.transpose()) + self.pod_centering_means
    ...
    def _warmup_upto_tinputminusone(self, inputs, training=False, usenoiseflag=False):
        ...
        ### Passing input through the GRU layers
        # first layer
        x = inputs[:, 0:-1, :] - self.pod_centering_means
        x = tf.linalg.matmul(x, self.Wl)
        x = self.normalization_preRNN(x)
    ...
    def train_step(self, data):
        ...
        with tf.GradientTape() as tape:
            ...
            ypred_renormalized = self.reverseNormalization_postPOD(ypred) - self.time_mean_ogdata

            ytrue_renormalized = self.reverseNormalization_postPOD(y) - self.time_mean_ogdata
    ...
    def save_everything(self, file_name, H5=True):
        ...
        np.savez(
            file_name+'_pod_matrices',
            Wl = self.Wl,
            pod_centering_means = self.pod_centering_means,
        )
