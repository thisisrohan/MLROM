...
class ESN_Cell(layers.Layer):
    ...
    def _build_Win(self):
        ...
        return Win.astype('float32'), Win_bias.astype('float32')
    ...
    def build(self, input_shape):
        ...
        self.Win = tf.layers.Dense(
            self.state_num_units,
            use_bias=self.usebias_Win,
            dtype='float32',
            name='Win'
        )
        self.Win.build(input_shape=(None, None, self.input_size)
        ...
        self.Wres = tf.layers.Dense(
            self.state_num_units,
            use_bias=False,
            dtype='float32',
            name='Wres'
        )
        self.Wres.build(input_shape=(None, self.state_num_units, self.state_num_units))
