################################################################################
# RK Methods inspired residual SimpleRNN with skip connections, with           #
# uniform/normal noise added to every input and learnable initial states.      #
# [STATEFUL]                                                                   #
# The output matrix uses cross-attention                                       #
#------------------------------------------------------------------------------#
...
class SimpleRNNCell_zoneout(layers.SimpleRNNCell):
    ...
    def build(self, input_shape):
        ...
        self.Wout_attention = self.add_weight(
            name="Wout_attention",
    ...
    def call(self, inputs, states, training=None):
        ...
        output, new_state = super().call(inputs, states, training)
        
        # prev_output = tf.expand_dims(prev_output, axis=-1)
        # output = tf.expand_dims(output, axis=-1)
        # attention_mat = tf.linalg.matmul(prev_output, output, transpose_b=True)
        
        attention_mat = []
        for i in range(inputs.shape[0]):
            attention_mat_i = tf.tensordot(prev_output[i, :], output[i, :], axes=0)
            attention_mat.append(attention_mat_i)
        attention_mat = tf.stack(attention_mat)
        
        attention_mat = attention_mat * self.Wout_attention
        attention_mat = tf.math.sigmoid(attention_mat)
        attention_mat = attention_mat / tf.math.reduce_sum(attention_mat, axis=-1, keepdims=True)
        # attention_corrected_output = tf.linalg.matmul(attention_mat, output)
        # attention_corrected_output = tf.squeeze(attention_corrected_output, axis=-1)
        attention_corrected_output = tf.squeeze(
            tf.linalg.matmul(
                attention_mat,
        ...
        # attention_corrected_output = []
        # for i in range(self.units):
        #     attention_mat_row = prev_output[:, i:i+1] * output
        #     attention_mat_row = attention_mat_row * self.Wout_attention[i, :]
        #     attention_mat_row = tf.math.exp(attention_mat_row)
        #     attention_mat_row = attention_mat_row / tf.math.reduce_sum(attention_mat_row, axis=-1, keepdims=True)
        #     attention_corrected_output_elem_i = tf.math.reduce_sum(attention_mat_row * output, axis=-1)
        #     attention_corrected_output.append(attention_corrected_output_elem_i)
        # attention_corrected_output = tf.stack(attention_corrected_output)
        # attention_corrected_output = tf.transpose(attention_corrected_output, [1, 0])
        
        # attention_corrected_output = []
        # for i in range(inputs.shape[0]):
        #     # doing each batch element's computation indivdually becaus of memeory constraints
        #     attention_mat_i = tf.tensordot(prev_output[i, :], output[i, :], axes=0)
        #     
        #     attention_mat_i = attention_mat_i * self.Wout_attention
        #     attention_mat_i = tf.math.exp(attention_mat_i)
        #     attention_mat_i = attention_mat_i / tf.math.reduce_sum(attention_mat_i, axis=-1, keepdims=True)
        #     # attention_corrected_output = tf.linalg.matmul(attention_mat, output)
        #     # attention_corrected_output = tf.squeeze(attention_corrected_output, axis=-1)
        #     attention_corrected_output_i = tf.squeeze(
        #         tf.linalg.matmul(
        #             attention_mat_i,
        #             tf.expand_dims(output[i, :], axis=-1),
        #         ),
        #         axis=-1
        #     )
        #     attention_corrected_output.append(attention_corrected_output_i)
        # attention_corrected_output = tf.stack(attention_corrected_output)
        
        new_state = [attention_corrected_output] if tf.nest.is_nested(states) else attention_corrected_output

        return attention_corrected_output, new_state
...
class RNN_GRU(Model):
    ...
    def __init__(
        ...
        if self.num_skip_connections > 0:
            self.RK_RNNCell = SimpleRNNCell_zoneout(
