...
class CrossAttentionSimpleRNNCell(layers.AbstractRNNCell):
    ...
    def call(self, inputs, states, training=None):
        ...
        h = tf.squeeze(tf.linalg.matmul(attention_mat, inputs), axis=-1)
        ...
        output = h + tf.squeeze(tf.linalg.matmul(recurrent_attention_mat, prev_output), axis=-1)
...
class RNN_CrossAttention(Model):
    ...
    def compute_metrics(self, x, y, y_pred, sample_weight):
        ...
        metric_results['gamma_prime_attention'] = 0.0
        metric_results['gamma_prime_recurrent_attention'] = 0.0
