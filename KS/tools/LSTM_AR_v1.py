################################################################################
# RK Methods inspired residual LSTM with skip connections, with uniform/normal #
# noise added to every input and learnable initial states. [STATEFUL]          #
# For LSTM_SingleStep_v1                                                       #
#------------------------------------------------------------------------------#
...
# a single layer network is the same as a regular LSTM.                        #
#                                                                              #
...
class LSTMCell_zoneout(layers.LSTMCell):
    ...
    def call(self, inputs, states, training=None):
        ...
        output, candidate_states = super().call(
            inputs,
            states,
            training
        )
        if 0.0 < self.zoneout_rate < 1.0:
            ...
            new_states = (h_t, candidate_states[1])
        else:
...
class AR_RNN_LSTM(Model):
    ...
    def __init__(
        ...
        if self.num_skip_connections > 0:
            self.RK_RNNCell = cell=LSTMCell_zoneout(
    ...
    def _warmup(self, inputs, training=None, usenoiseflag=False, **kwargs):
        ...
        returned_tuple = self.rnn_list[0](
            x,
            training=training,
        )
        x = returned_tuple[0]
        states = returned_tuple[1:]
        intermediate_outputs_lst.append(x)
        states_list.append(states)

        # remaining layers
        for i in range(self.num_skip_connections):
            returned_tuple = self.rnn_list[i+1](
                x,
                training=training,
            )
            prediction = returned_tuple[0]
            states = returned_tuple[1:]
    ...
    def save_everything(self, file_name, H5=True):
        ...
        ### saving weights
        self.save_model_weights(file_name+'_rnn_weights', H5=H5)
