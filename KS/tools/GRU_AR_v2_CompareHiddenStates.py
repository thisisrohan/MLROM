...
class AR_RNN_GRU_CHS(Model):
    ...
    def __init__(
        ...
        super(AR_RNN_GRU_CHS, self).__init__()
    ...
    def call(self, inputs, training=None):
        predictions_AR_list = []
        hidden_states_AR_list = []
        
        predictions_TF_list = []
        hidden_states_TF_list = []
        
        prediction_AR, states_list_AR = self._warmup(inputs, training)
        prediction_TF, states_list_TF = self._warmup(inputs, training)

        # first prediction
        predictions_AR_list.append(prediction_AR)
        predictions_TF_list.append(prediction_TF)
        
        # print(type(states_list))
        hidden_states_AR_list.append(states_list_AR)
        hidden_states_TF_list.append(states_list_TF)

        ### Run the rest of the prediction steps -- AR mode
        for i in range(1, self.out_steps):
            for j in range(self.num_rnn_layers):
                prediction_AR, *states_AR = self.rnn_cells_list[j](
                    prediction_AR,
                    states=states_list_AR[j],
                    training=training
                )
                states_list_AR[j] = states_AR[0]
            prediction_AR = self.dense(prediction_AR, training=training)
            predictions_AR_list.append(prediction_AR)
            hidden_states_AR_list.append(states_list_AR)

        ### Run the rest of the prediction steps -- TF mode
        # states_list_TF = hidden_states_TF_list[0]
