...
class LSTMCell_zoneout(layers.LSTMCell):
    ...
    def call(self, inputs, states, training=None):
        ...
        if 0.0 < self.zoneout_rate < 1.0:
            h_tcandidate = candidate_states[0]
            c_tcandidate = candidate_states[0]            
            if training == True:
                ...
                c_t = c_tm1 + zoneout_mask * (c_tcandidate - c_tm1)
            else:
                h_t = h_tcandidate + self.zoneout_rate * (h_tm1 - h_tcandidate)
                c_t = c_tcandidate + self.zoneout_rate * (c_tm1 - c_tcandidate)
            # h_t = [h_t] if tf.nest.is_nested(states) else h_t
        else:
            h_t = candidate_states[0]
            c_t = candidate_states[1]
...
class AR_RNN_LSTM(Model):
    ...
    def __init__(
        ...
        if self.num_skip_connections > 0:
            self.RK_RNNCell = LSTMCell_zoneout(
        ...
        self.init_state = [[None, None]]*len(self.rnn_layers_units)
        # self.init_state = []
