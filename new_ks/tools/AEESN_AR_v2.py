...
class AR_AERNN_ESN(Model):
    ...
    def _call_for_train(self, _warmup_upto_tinputminusone_tuple, training=False):
        predictions_list = []
        first_step_pred = _warmup_upto_tinputminusone_tuple[0]
        first_step_pred = first_step_pred[:, -1:, :]
        predictions_list.append(first_step_pred)
        states_list = _warmup_upto_tinputminusone_tuple[1]
        if len(_warmup_upto_tinputminusone_tuple) > 2:
            intermediate_outputs_lst = _warmup_upto_tinputminusone_tuple[2]
            scalar_multiplier_list = _warmup_upto_tinputminusone_tuple[3]
        else:
            intermediate_outputs_lst = None
            scalar_multiplier_list = None

        ### Passing input through the GRU layers
        x = predictions_list[-1]
        for tstep in range(1, self.rnn_net.out_steps):
            x = layers.TimeDistributed(self.ae_net.encoder_net)(x, training=training)
            x = self.normalization_preRNN(x)
    ...
    def train_step(self, data):
        ...
        with tf.GradientTape() as tape:
            ypred = self._call_for_train(warmup_tuple, training=True)
